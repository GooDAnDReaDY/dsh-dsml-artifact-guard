window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      modelPattern: 'Model Pattern (RegExp)',
      modelPatternHint: 'Case-insensitive RegExp matched against model IDs (e.g. deepseek, deepseek-v4.*). Default: deepseek.',
      providers: 'Target Providers (optional)',
      providersPlaceholder: 'all providers (comma-separated, e.g. commandcode, deepseek-official)',
      providersHint: 'Comma-separated list of provider IDs. Leave empty to guard all providers.',
      legacyDetected: 'Legacy providerId / modelId detected: active as an additional rule.',
      legacyClear: 'Clear legacy fields',
      providerId: 'Target Provider ID (legacy)',
      modelId: 'Target Model ID (legacy)',
    ...
    const zh = {
      ...
      modelPattern: '模型正则匹配式 (RegExp)',
      modelPatternHint: '对模型 ID 进行不区分大小写的正则表达式匹配（默认：deepseek）。支持任意服务商前缀。',
      providers: '目标服务商（可选）',
      providersPlaceholder: '所有服务商（以逗号分隔，例如 commandcode, deepseek-official）',
      providersHint: '以逗号分隔的服务商 ID 列表。留空则保护所有服务商。',
      legacyDetected: '检测到旧版 providerId / modelId 配置，并作为附加精确规则生效。',
      legacyClear: '清除旧版配置',
      providerId: '目标服务商标识 (Provider ID, 旧版)',
      modelId: '目标模型标识 (Model ID, 旧版)',
    ...
    const ru = {
      title: 'Защита от артефактов DSML',
      sub: 'Фильтрация утекших замыкающих тегов протокола DSML из потоков моделей DeepSeek',
      version: 'Версия',
      ...
      updating: 'Обновление…',
      updateSuccess: 'Обновлено до v{v}. Перезапустите службу DSH для применения изменений.',
      updateFailed: 'Ошибка обновления: {msg}',
      checkUpdateFailed: 'Не удалось проверить обновления',
      mode: 'Режим работы',
      modeSanitize: 'Санация (вырезать утекшие замыкающие теги)',
      modeAudit: 'Аудит (только логировать предупреждения в журнал)',
      modeDisabled: 'Отключено (пропускать все потоки без изменений)',
      modelPattern: 'Шаблон модели (RegExp)',
      modelPatternHint: 'Регулярное выражение без учёта регистра для ID модели (по умолчанию: deepseek).',
      providers: 'Целевые провайдеры (опционально)',
      providersPlaceholder: 'все провайдеры (через запятую, напр. commandcode, deepseek-official)',
      providersHint: 'Список ID провайдеров через запятую. Оставьте пустым для защиты всех провайдеров.',
      legacyDetected: 'Обнаружены устаревшие параметры providerId / modelId (действуют как дополнительное правило).',
      legacyClear: 'Очистить устаревшие поля',
      providerId: 'Целевой провайдер (устаревшее)',
      modelId: 'Целевая модель (устаревшее)',
      disabledNotice: '⚠️ Защита отключена. Замыкающие теги DSML будут попадать в интерфейс чата.',
      statusUnavailable: 'Служба настроек недоступна.',
      saving: 'Сохранение…',
      ready: 'Сохранить настройки',
    ...
    function PluginCard({ ctx: _ctx, t, bare }) {
      ...
      if (!scopeRef.current && _ctx) {
        try {
          if (_ctx.configForms && typeof _ctx.configForms.get === 'function') {
            scopeRef.current = _ctx.configForms.get(ROW_ID) || _ctx.configForms.get(NS);
          } else if (_ctx.settingsScope && typeof _ctx.settingsScope.bind === 'function') {
      ...
      const rawLang = ((typeof _ctx?.locale?.getLocale === 'function' ? _ctx.locale.getLocale() : null) || (typeof navigator !== 'undefined' && navigator.language) || 'en');
      const currentLang = rawLang.startsWith('ru') ? 'ru' : (rawLang.startsWith('zh') ? 'zh' : 'en');
      const dict = currentLang === 'ru' ? ru : (currentLang === 'zh' ? zh : en);
      const tt = (k, params) => {
      ...
      // Load settings from snapshot and subscribe to live changes
      React.useEffect(() => {
        if (!scope) {
        ...
        let cancelled = false;
        const syncSnapshot = (snap) => {
          if (!snap || cancelled) return;
          const view = snap.view || snap.value || snap;
          if (typeof snap === 'object' && 'status' in snap && snap.status !== 'ready') {
            setStatus(snap.status);
            return;
          }
          if (typeof view === 'object' && view !== null) {
            setDraft({
              mode: view.mode || DEFAULTS.mode,
              modelPattern: view.modelPattern !== undefined ? String(view.modelPattern) : DEFAULTS.modelPattern,
              providers: Array.isArray(view.providers)
                ? view.providers.join(', ')
                : (typeof view.providers === 'string' ? view.providers : ''),
              providerId: view.providerId ? String(view.providerId) : '',
              modelId: view.modelId ? String(view.modelId) : '',
            });
            setStatus('ready');
          } else {
            setStatus('ready');
        ...
        (async () => {
          try {
            const snap = typeof scope.getSnapshot === 'function' ? scope.getSnapshot() : (typeof scope.get === 'function' ? await scope.get() : null);
            syncSnapshot(snap);
          } catch (err) {
        ...
        if (typeof scope.subscribe === 'function') {
          try {
            unsubscribe = scope.subscribe(() => {
              try {
                syncSnapshot(typeof scope.getSnapshot === 'function' ? scope.getSnapshot() : null);
              } catch (err) {
                /* Snapshot update read failed */
              }
            });
          } catch (err) {
            /* Subscription to scope updates unavailable */
          }
        }
      ...
      const onSave = async () => {
        ...
        try {
          const failures = [];
          const entries = [
            ['mode', draft.mode || 'sanitize'],
            ['modelPattern', (draft.modelPattern !== undefined && draft.modelPattern !== null) ? String(draft.modelPattern).trim() : 'deepseek'],
            ['providers', draft.providers ? draft.providers.split(',').map(s => s.trim()).filter(Boolean) : []],
            ['providerId', draft.providerId ? String(draft.providerId).trim() : ''],
            ['modelId', draft.modelId ? String(draft.modelId).trim() : ''],
          ];
          for (const [key, val] of entries) {
            try {
              await scope.set(key, val);
            } catch (err) {
          ...
          status === 'ready' && React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 14 } },
            ...
            React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 6 } },
              ...
              React.createElement('input', {
                ...
                onChange: (e) => setDraft(d => ({ ...d, modelPattern: e.target.value })),
                style: {
                  height: 34,
                  borderRadius: 6,
                  border: '1px solid var(--dsw-alias-border-l2)',
              ...
              React.createElement('span', {
                style: { fontSize: 11, color: 'var(--dsw-alias-label-tertiary)', lineHeight: '1.4' },
              }, tt('modelPatternHint'))
            ),

            // Target Providers Field
            React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 6 } },
              React.createElement('label', {
                style: { fontSize: 13, fontWeight: 500, color: 'var(--dsw-alias-label-primary)' },
              }, tt('providers')),
              React.createElement('input', {
                ...
                placeholder: tt('providersPlaceholder'),
                onChange: (e) => setDraft(d => ({ ...d, providers: e.target.value })),
                style: {
                  height: 34,
                  borderRadius: 6,
                  border: '1px solid var(--dsw-alias-border-l2)',
            ...
            // Legacy Provider/Model notice if detected
            (draft.providerId || draft.modelId) && React.createElement('div', {
              style: {
                display: 'flex',
                ...
                border: '1px solid color-mix(in srgb, var(--dsw-alias-state-warning-primary) 25%, transparent)',
                fontSize: 12,
                color: 'var(--dsw-alias-label-secondary)',
              },
            },
              React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 2 } },
                React.createElement('span', null, tt('legacyDetected')),
                React.createElement('span', { style: { fontFamily: 'monospace', fontSize: 11 } },
              ...
              React.createElement('button', {
                type: 'button',
                onClick: () => setDraft(d => ({ ...d, providerId: '', modelId: '' })),
                style: {
    ...
    function PluginItem(props) {
      if (props && props.view === 'summary') {
        const tBind = (props && props.ctx && typeof props.ctx.locale?.bind === 'function')
          ? props.ctx.locale.bind(NS)
          : null;
        const tt = typeof props.t === 'function' ? props.t : (typeof tBind === 'function' ? tBind : (k) => en[k] || k);
        return React.createElement('div', {
          'data-dsh-plugin': NS,
    ...
    exports.apply = function apply(ctx) {
      if (typeof ctx.locale?.register === 'function') {
        const registerLocale = () => {
          try {
            return ctx.locale.register(NS, { en, zh, ru });
          } catch (err) {
            /* Locale namespace already registered or unavailable */
          }
        };
        if (typeof ctx.effect === 'function') {
          ctx.effect(registerLocale, 'dsh-dsml-artifact-guard: locale');
        } else {
          registerLocale();
        }
      }
