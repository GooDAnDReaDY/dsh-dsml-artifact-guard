window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PluginCard({ ctx: _ctx, t, bare }) {
      ...
      const currentLang = ((typeof _ctx?.locale?.getLocale === 'function' ? _ctx.locale.getLocale() : null) || (typeof navigator !== 'undefined' && navigator.language) || 'en').startsWith('zh') ? 'zh' : 'en';
      ...
      React.useEffect(() => {
        ...
        const syncSnapshot = (snap) => {
          ...
          if (typeof snap === 'object' && 'status' in snap) {
            if (snap.status === 'ready' && view) {
              setDraft({
                mode: view.mode || DEFAULTS.mode,
                providerId: view.providerId || DEFAULTS.providerId,
                modelId: view.modelId || DEFAULTS.modelId,
              });
              setStatus('ready');
            } else {
